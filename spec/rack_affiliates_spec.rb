...
describe "RackAffiliates" do
  ...
  it "should handle empty reqest affiliate info" do
    get '/'

    last_request.env['req.from'].must_equal nil
    last_request.env['req.time'].must_equal nil
  end

  it "should set reqest affiliate info from params" do
    Timecop.freeze do
      @time = Time.now
      get '/', 'HTTP_REFERER' => "http://www.foo.com"
    end
  ...
  describe "when cookie exists" do
    ...
    it "should use newer affiliate" do
      Timecop.freeze(60*60*24) do #1 day later
        @new_time = Time.now
        get '/', 'HTTP_REFERER' => "http://www.bar.com"
      end
